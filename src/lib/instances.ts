...
// remote-control is launched with --permission-mode bypassPermissions (the
// actual flag claude remote-control accepts — the --dangerously-skip-
// permissions flag is for plain `claude`, not remote-control). Any string
// we interpolate from the DB is shQuoted; session name is generated by us
// so always safe.
export function buildSpawnCommand(opts: {
  ...
  const inner = `exec claude remote-control --permission-mode bypassPermissions --spawn same-dir --name ${name} ${extra}`.trim();
  return (
