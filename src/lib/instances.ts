...
export async function spawnInstance(instanceId: number, d?: Db, opts?: SpawnOptions): Promise<SpawnResult> {
  ...
  // Write .dcm/system-prompt.md with hosts + optional workflow, then pass it
  // via --append-system-prompt-file so Claude reads it reliably.
  const extraClaudeArgs: string[] = [];
  if (inst.host_id === null) {
    try {
      const { writeSystemPromptFile } = await import("./host-context");
      const promptFile = await writeSystemPromptFile(project.path_on_host, db, {
        includeWorkflow: opts?.useWorkflow,
      });
      extraClaudeArgs.push("--append-system-prompt-file", promptFile);
    } catch {
      // non-fatal — instance still spawns, just without host context
    }
  }
