...
function buildHostContext(db: Db): string {
  ...
  return [
    ...
    "This list may update during your session — check .dcm/hosts.md in your project directory for the latest.",
    ...lines,
  ].join("\n");
}
...
export async function spawnInstance(instanceId: number, d?: Db, opts?: SpawnOptions): Promise<SpawnResult> {
  const db = d ?? getDb();
  ...
  if (opts?.useWorkflow) {
    extraClaudeArgs.push("--append-system-prompt", WORKFLOW_PROMPT);
  }

  // Write .dcm/hosts.md into the project dir so Claude can read latest hosts.
  if (inst.host_id === null) {
    const { updateHostFiles } = await import("./host-context");
    void updateHostFiles(db);
  }
